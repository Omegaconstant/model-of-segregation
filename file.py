# Run Command: streamlit run file.py
...
# Creat Schelling Class:
# A matrix of people of 2 kinds of community and some spaces, where everyone try to move towards a place where no. of negihbour of his community is larger than certain fractional thershold
# We try to study over time effect of this process
class Schelling:
            ...
            if race != 0:
                ...
                if neighborhood_size != n_empty_houses + 1:
                    ...
                    similarity_ratio = n_similar / (
                        neighborhood_size - n_empty_houses - 1.0
                    )
                    is_unhappy = similarity_ratio < self.similarity_threshold
                    if is_unhappy:
                        empty_houses = list(
                            zip(
                                np.where(self.city == 0)[0], np.where(self.city == 0)[1]
                            )
                        )
